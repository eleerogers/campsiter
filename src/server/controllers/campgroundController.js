...
const getCampgrounds = (request, response, next) => {
  pool.query('SELECT campgrounds.id, campgrounds.name, campgrounds.image, campgrounds.description, campgrounds.user_id, campgrounds.price, campgrounds.location, campgrounds.lat, campgrounds.lng, campgrounds.created_at, campgrounds.image_id, ycusers.email FROM campgrounds LEFT OUTER JOIN ycusers ON campgrounds.user_id = ycusers.id ORDER BY campgrounds.id ASC', (error, results) => {
    if (error) {
      console.error(error);
